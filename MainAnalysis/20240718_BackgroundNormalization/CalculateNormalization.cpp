...
int main(int argc, char *argv[])
{
   ...
   for(int i = 1; i <= NBin; i++)
   {
      ...
      {
         cout << "ERROR" << endl;
         // cout << HN0->GetBinContent(i) << " " << HDY->GetBinContent(i) << endl;
         continue;
      }
      ...
      // cout << HDY->GetBinCenter(i) << " " << HDY->GetBinContent(i) << " " << A1 << " " << A2 << endl;

      AnswerBBSame = AnswerBBSame + HDY->GetBinContent(i) * (A2 - AA);
      AnswerBBDiff = AnswerBBDiff + HDY->GetBinContent(i) * A1 * A1;
   }
   ...
   cout << "BxB (diff) = " << AnswerBBDiff << " " << AnswerBBDiff / NZ << endl;
